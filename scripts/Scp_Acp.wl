...
(*============== Script for A_CP(pi0 K_S) and S_CP(pi0 K_S) contours ==============*)
...
Quiet @ Check[
  ...
  Get[FileNameJoin[{projectDir, "src", "uncertainty.wl"}]];

  (* -------- local helpers from notebook migration -------- *)
  ClearAll[realClean];
  realClean[x_, tol_ : 10^-10] :=
   Module[{xc = Chop[x, tol]},
  ...
  ClearAll[phi00sol];
  phi00sol[input_Association, A_?NumericQ] :=
   Module[
    {
      A00, Amp, A32, A00bar, Ampbar, A32bar,
      phi32, phi1, phi2, phi001, phi002, phi003, phi004
    },
  ...
  ClearAll[ScpBranch];
  ScpBranch[input_Association, A_?NumericQ, k_Integer] :=
   Module[{phi00k},
  ...
  ComputeScpAcpCenter[input_Association, OptionsPattern[]] :=
   Module[{aRange, aStep, as, curves},
    ...
    curves = Association @ Table[
      With[{branch = k},
        "Branch" <> ToString[branch] ->
          Table[
            {A, realClean @ ScpBranch[input, A, branch]},
            {A, as}
          ]
      ],
      {k, 1, 4}
  ...
  BuildScpAcpFigure[centerRes_Association, unc_Association, inputs_Association, OptionsPattern[]] :=
   Module[
    {
      curves, bands,
      pts1, pts2, pts3, pts4,
      ...
      xMajor, xMinor, yMajor, yMinor, ticksX, ticksY, frameTicksTarget,
      xLabel, yLabel, axisLikeTarget,
      xL, xR, sumRuleBandEpilog, sumRuleTextEpilog,
      currentDataEpilog, currentDataTextEpilog,
      colBlue, colGreen, colOrange, colGray,
      leftPlot, rightPlot, imgSingle, imgRow
    },

    curves = centerRes["Curves"];
    bands = unc["Curves"]["Bands"];

    pts1 = curves["Branch1"]; pts2 = curves["Branch2"];
    pts3 = curves["Branch3"]; pts4 = curves["Branch4"];

    sig1 = bands["Branch1"]; sig2 = bands["Branch2"];
    sig3 = bands["Branch3"]; sig4 = bands["Branch4"];
    ...
    mkFrameTicks[major_, minor_, labfmt_, majLen_, minLen_] :=
     Module[{minorOnly, clean},
    ...
    currentDataTextEpilog = {
      Inset[
        Style["Current\ndata", 18, Black, FontFamily -> "Times"],
        {Acppi0Ks0 + 0.02, Scppi0Ks0 - 0.15},
        {Left, Center}
      ]
    };
    ...
    ClearAll[bandPoly];
    bandPoly[centerPts_, sigmaBand_] :=
     Module[{pairs, up, dn},
      pairs = Select[
    ...
    leftPlot = ListLinePlot[
      ...
      Epilog -> Join[
        sumRuleBandEpilog, sumRuleTextEpilog,
        currentDataEpilog, currentDataTextEpilog
      ],
      ImageSize -> imgSingle
    ];
  ...
  compute = Function[in, ComputeScpAcpCenter[in, "ARange" -> aRange, "AStep" -> aStep]];
  scalarExtractor = Function[res, <|"NPoints" -> Length[res["GridA"]]|>];
  curveExtractor = Function[res, res["Curves"]];

  Print["[INFO] Building central A_CP-S_CP contours"];
  centerRes = compute[centralInputs];

  Print["[INFO] Propagating finite-difference 1-sigma bands"];
  uncRes = ComputeUncertaintyFD[
    compute, scalarExtractor, curveExtractor,
    centralInputs, paramSpecs
  ];

  fig = BuildScpAcpFigure[
    centerRes,
    uncRes,
    centralInputs,
    "PlotRange" -> plotRange
  ...
  (* -------- output directory -------- *)
  outRoot = FileNameJoin[{projectDir, "results", "Scp_Acp"}];
  ...
  base = FileNameJoin[{outDir, "Scp_Acp"}];
  ...
  Export[
    ...
    StringRiffle[
      {
        "Scp_Acp export",
        "ARange = " <> ToString[aRange, InputForm],
        "AStep = " <> ToString[aStep],
        "PlotRange = " <> ToString[plotRange, InputForm],
        "CurrentData = {" <> ToString[N[Acppi0Ks0, 8]] <> ", " <> ToString[N[Scppi0Ks0, 8]] <> "}",
        "SMBand = {" <> ToString[N[Acppi0KsSM0 - Acppi0KsSMErr, 8]] <> ", " <> ToString[N[Acppi0KsSM0 + Acppi0KsSMErr, 8]] <> "}"
      },
      "\n"
    ],
    "String"
